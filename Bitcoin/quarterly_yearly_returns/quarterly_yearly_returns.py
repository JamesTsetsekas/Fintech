#!/usr/bin/env python3
"""
Bitcoin Quarterly & Yearly Returns Visualization

Creates a table visualization showing quarterly and yearly returns for Bitcoin
...
# Calculate quarterly returns (from Open to Close of the quarter)
quarterly_data = quarterly_data.sort_values(['Year', 'Quarter'])
quarterly_data['Return'] = ((quarterly_data['Close'] - quarterly_data['Open']) / quarterly_data['Open']) * 100
...
years = sorted(quarterly_data['Year'].unique(), reverse=True)[:10]
...
# Table dimensions
cell_width = 2.0  # Wider cells for quarters
cell_height = 0.8
year_label_width = 0.8
year_label_x = -0.5  # Moved further left to avoid overlap
table_x_start = 1.2  # Moved right to create spacing from year column
table_y_start = 8.5
...
for i, quarter in enumerate(quarters):
    ...
    quarter_x_positions[i+1] = x_pos
    rect = Rectangle((x_pos - cell_width/2, header_y - cell_height/2), 
                     cell_width, cell_height, 
                     linewidth=1.5, edgecolor='white', facecolor='#1a1a1a')
    ax.add_patch(rect)
    ax.text(x_pos, header_y, quarter, ha='center', va='center', 
            fontsize=11, fontweight='bold', color='white')

# Yearly column header
yearly_x = table_x_start + 4 * cell_width
...
for idx, year in enumerate(years):
    ...
    year_quarterly = quarterly_data[quarterly_data['Year'] == year]
    
    for quarter_num in range(1, 5):
        quarter_data = year_quarterly[year_quarterly['Quarter'] == quarter_num]
        x_pos = quarter_x_positions[quarter_num]
...
ax.text(table_x_start + 2 * cell_width, title_y, 
        'Quarterly & Yearly Returns (Last 10 Years)', 
        ha='center', va='center', fontsize=18, fontweight='bold', color='white')

# Set limits - adjusted to accommodate year column on the left and 10 years of data
ax.set_xlim(-1.2, table_x_start + 5 * cell_width + 1)
...
plt.tight_layout()
output_path = Path(__file__).parent / 'quarterly_yearly_returns.png'
