...
linear_regression_fit_extended = np.exp(intercept) * (extended_days_since_genesis ** slope)

# --- 2. Define Support and Resistance Bands (Illustrative) ---
std_dev = np.std(log_price - (intercept + slope * log_days))
upper_band_extended = np.exp(intercept + (2 * std_dev)) * (extended_days_since_genesis ** slope)
lower_band_extended = np.exp(intercept - (2 * std_dev)) * (extended_days_since_genesis ** slope)
...
plt.plot(smooth_days, smooth_linear_fit, color='red', linestyle='-', label='Linear Regression (Projected)')
...
plt.title('Bitcoin Price with Linear Regression and Potential Bands (Projected to 2040, Log-Log)', color='gray')
